...
pbpk <- function(){
  ...
  model({
    ...
    KbMU = exp(lKbMU + eta.LKbMU)
    KbAD = exp(lKbAD + eta.LKbAD)
    CLint= exp(lCLint + eta.LClint)
    KbBO = exp(lKbBO + eta.LKbBO)
    KbRB = exp(lKbRB + eta.LKbRB)
...
sim <- vpcSim(fit.addProp.S, n = n_sim)
...
fitdf <- as.data.frame(fit.addProp.S)
