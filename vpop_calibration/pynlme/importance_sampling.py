...
class ImportanceSampler:
    ...
    def fit_student_t_proposal(
        ...
        clamped_variances = torch.clamp(torch.var(etas, 0), 1e-6)
        standard_deviations = torch.sqrt(clamped_variances)

        self.dist = dist.StudentT(
            df=torch.tensor(self.df, device=device), loc=mu, scale=standard_deviations
        )
