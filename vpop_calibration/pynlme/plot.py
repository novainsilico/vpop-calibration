...
from vpop_calibration.pynlme.utils import inverse_transform_param, theoretical_pdf
...
class PriorVisualizer:
    ...
    def compare(self, log_scale: bool = True):
        ...
        init_beta = torch.as_tensor(model.prior_params.beta_init)
        if torch.equal(model.population_betas, init_beta):
            raise RuntimeError(
                "SAEM has not been launched yet. Please use model.optimizer.run()"
            )
