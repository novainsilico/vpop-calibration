...
def check_surrogate_validity_gp(
    model_diag: ModelDiagnostics,
    scaling_indiv_plot: float = 3.0,
    scaling_2by2_plot: float = 2.0,
    n_columns: int = 3,
) -> tuple[dict, dict]:
    pdus = model_diag.model.descriptors
    gp_model_struct = model_diag.model.structural_model
    assert isinstance(
        gp_model_struct, StructuralGp
    ), "Posterior surrogate validity check only implemented for GP structural model."

    if model_diag.individual_ebe_estimates_df is None:
        model_diag.compute_ebe()
    assert model_diag.individual_ebe_estimates_df is not None
    gp_model: GP = gp_model_struct.gp_model
    train_data = gp_model.data.full_df_raw[pdus].drop_duplicates()

    map_data = model_diag.individual_ebe_estimates_df
    patients = model_diag.model.patients
...
def plot_map_estimates(
    model_diag: ModelDiagnostics, facet_width: float = 5.0, facet_height: float = 4.0
) -> None:
    if model_diag.individual_ebe_predictions_df is None:
        model_diag.compute_ebe()
    assert model_diag.individual_ebe_predictions_df is not None
    obs_vs_simulated = model_diag.individual_ebe_predictions_df

    n_cols = model_diag.model.nb_outputs
    n_rows = model_diag.model.nb_protocols
    _, axes = plt.subplots(
        n_rows,
        n_cols,
    ...
    colors = cmap(np.linspace(0, 1, model_diag.model.nb_patients))
    for output_index, output_name in enumerate(model_diag.model.output_names):
        for protocol_index, protocol_arm in enumerate(model_diag.model.protocol_arms):
            ...
            for patient_ind in patients_protocol:
                patient_num = model_diag.model.patients.index(patient_ind)
...
def plot_individual_map_estimates(
    ...
    if patient_num is None:
        total_patient_num = model_diag.model.nb_patients
    ...
    colors = cmap(np.linspace(0, 1, len(model_diag.model.output_names)))
...
def plot_all_individual_map_estimates(
    ...
    if n_patients_to_plot is None or n_patients_to_plot > model_diag.model.nb_patients:
        n_patients_to_plot = model_diag.model.nb_patients

    print(
        f"There are {model_diag.model.nb_patients} patients in total. {n_patients_to_plot} will be plotted."
    )
    ...
    if randomize:
        ind_to_plot = rand.sample(
            range(model_diag.model.nb_patients), n_patients_to_plot
        )
    else:
        ind_to_plot = list(range(n_patients_to_plot))

    cmap = plt.get_cmap("brg")
    colors = cmap(np.linspace(0, 1, model_diag.model.nb_outputs))
    ...
    for output_index, output_name in enumerate(model_diag.model.output_names):
        ...
        for k in range(0, n_patients_to_plot):
            ...
            patient_ind = model_diag.model.patients[ind_to_plot[k]]
...
def plot_map_estimates_gof(
    ...
    num_plots = model_diag.model.nb_outputs
    fig, axes = plt.subplots(
    ...
    for output_index, output_name in enumerate(model_diag.model.output_names):
        ...
        ax = axes[0, output_index]
        gof_df = obs_vs_simulated.loc[(obs_vs_simulated["output_name"] == output_name)]
...
def plot_weighted_residuals(
    ...
    match res_type:
        case "pwres":
            if model_diag.pwres is None:
                model_diag.compute_pwres()
            assert model_diag.pwres is not None
            wres_results = model_diag.pwres
        ...
        case "npde":
            if model_diag.npde is None:
                model_diag.compute_npde()
            assert model_diag.npde is not None
            wres_results = model_diag.npde
            compare_to_pop_pred = True
        case _:
            raise ValueError(f"Not implemented residual type: {res_type}")
    if compare_to_pop_pred:
        if model_diag.population_parameters_predictions_df is None:
            model_diag.zero_random_effect_predictions()
...
def plot_map_vs_posterior(
    ...
    if model_diag.conditional_distribution_samples is None:
        model_diag.sample_conditional_distribution()
    assert model_diag.conditional_distribution_samples is not None
    sample_etas = model_diag.conditional_distribution_samples

    sample_gaussian = model_diag.model.convert_etas_to_gaussian_all_patients(
        sample_etas
    )
    sample_physical = model_diag.model.convert_gaussian_to_physical(
        psi=sample_gaussian, log_mi=model_diag.model.log_mi
    )

    if n_patients_to_plot > model_diag.model.nb_patients:
        n_patients_to_plot = model_diag.model.nb_patients

    ind_to_plot = rand.sample(range(model_diag.model.nb_patients), n_patients_to_plot)

    # Get EBE estimates for descriptors
    if model_diag.individual_ebe_estimates_tensor is None:
        model_diag.compute_ebe()
    ebe_theta = model_diag.individual_ebe_estimates_tensor
    ...
    for k in range(n_patients_to_plot):
        ...
        n_cols = 3
        n_rows = (model_diag.model.nb_pdu + n_cols - 1) // n_cols
        ...
        for i in range(len(axes)):
            ...
            if i < model_diag.model.nb_pdu:
                ...
                ax.set_title(
                    f"Patient {ind_to_plot[k]} - {model_diag.model.pdu_names[i]}"
                )
