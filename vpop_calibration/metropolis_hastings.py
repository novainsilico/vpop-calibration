...
def mh_step(
    ...
    # previous_state.etas has shape (nb_chains, nb_patients, nb_pdu)
    # omega_pop_lower_chol is the lower triangular matrix of the cholesky decomposition
    # it has shape (nb_pdu, nb_pdu)
    # Because, for a given chain and patient, the etas is a ROW vector,
    # so to get correlated vectors with covariance LL^T, what we want here is eta @ L^T , NOT eta @ L
    proposal_noise = (
        torch.randn_like(previous_state.etas, device=device)
        @ nlme_model.omega_pop_lower_chol.T
    )
