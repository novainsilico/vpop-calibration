...
class NlmeModel:
    ...
    def calculate_residuals(
        ...
        ), "Non-matching shapes in `calculate_residuals`"
    ...
    def log_likelihood_observation(
        ...
        residuals: torch.Tensor = self.calculate_residuals(
            self.full_obs_data.expand(self.num_chains, -1), predictions
        )
        res_error_var = self.residual_var.expand(self.num_chains, -1).index_select(
        ...
        log_lik_per_patient = torch.zeros(
            (self.num_chains, self.nb_patients), device=device
        )
        log_lik_per_patient.scatter_add_(
            1,
            self.observation_to_patient_index.expand(self.num_chains, -1),
            log_lik_full,
        )
