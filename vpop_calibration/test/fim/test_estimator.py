...
def test_fim_estimator(sample_model):
    estimator = FimEstimator(sample_model)

    new_etas = sample_model.sample_etas(1)
    estimator.accumulate(new_etas)
    estimator.show_summary()
    estimator.show_covariance_matrix()
    estimator.show_fim()
    estimator.show_rse()
