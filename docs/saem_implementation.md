...
SAEM is an iterative algorithm and its implementation here is seperated into three phases: **burn-in**, **learning** and **smoothing**, each phase itself being composed of several iterations.
...
The three phases differ in how these two steps are orchestrated:
  * **burn-in**: runs only MCMC transitions while keeping population parameters fixed (M-step is skipped)
  * **learning**: updates both individual Gaussian parameters and population parameters (both E-step and M-step are run). The stochastic approximation estimates (see sub-section "1.2 Stochastic approximation") are updated with $\gamma_k = 1$, meaning there is **no averaging** with their previous values.
...
```math
\left(\sum_i {X_i}^T X_i\right)\hat\beta_{k+1} = s_{k+1}^{\text{cross}} \tag{2}
```
...
```math
\hat \sigma_{k+1}^2 = \frac{1}{N_{obs}} \sum_i \sum_j \left[y_{i,j} - f(\theta^{(k+1)}_i, t_{i,j})\right]^2
```
During the learning phase, the implementation applies a deterministic decay limiter to prevent the residual variance from decreasing too quickly. For each component of the variance, the target is first replaced by

```math
\tilde \sigma_{k+1}^{2} = \max\left(\hat \sigma_{k+1}^2,\alpha \sigma_{k}^2\right).
```
where $\alpha$ is the `annealing_factor` attribute of `SaemConfigDict`.
...
#### 2.1 Population effects $\beta$

The provisional value computed in (2) becomes the new population effects estimate:
```math
\beta_{k+1} = \hat \beta_{k+1}
```

#### 2.2 Covariance matrix $\Omega$


The target covariance matrix $\Omega$ reads

```math
\hat \Omega_{k+1} = \frac{1}{n} s_{k+1}^{\text{outer}}
```

Let $\tilde\Omega_{k+1}$ denote the covariance obtained after clamping the eigenvalues of $\hat\Omega_{k+1}$ to $10^{-6}$. During the learning phase, its target diagonal variances are replaced by

```math
d_{k+1,j}
=
\max\left((\tilde\Omega_{k+1})_{jj},
\alpha(\Omega_k)_{jj}\right)
```
to prevent the marginal variances from decreasing too quickly.
The final covariance matrix is reconstructed from these adjusted variances and the correlation matrix of $\tilde\Omega_{k+1}$:

```math
\Omega_{k+1}
=
\mathrm{diag}\left(\sqrt{d_{k+1,j}}\right)\operatorname{Corr}(\tilde\Omega_{k+1})\mathrm{diag}\left(\sqrt{d_{k+1,j}}\right)
```
During the smoothing phase, this decay limiter is disabled and
```math
\Omega_{k+1}=\tilde\Omega_{k+1}
```


#### 2.3 Model intrinsic and survival coefficients $\psi$ update

The MI parameters target is computed by maximizing the complete-data log-likelihood, all the random effects remaining fixed.
